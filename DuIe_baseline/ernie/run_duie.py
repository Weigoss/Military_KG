...
def predict_wrapper(reader, exe, test_prog, test_pyreader, graph_vars, epoch,
    ...
    for test_f, save_f in zip(test_sets, save_dirs):
        ...
        res, predict_time = predict(args, examples, exe, test_prog, test_pyreader, graph_vars)
        print(predict_time)
        ...
        with codecs.open(save_path, 'w', 'utf-8') as f:
            for result in res:
                ...
                # for spo in result['spo_list']:
                    # print('SPO_LIST:\tSubject:{}\tPredicate:{}\tSubject:{}\t'.format(spo['subject'],spo['predicate'],spo['object']['@value']))
