    ...
    while True:
        try:
            ...
            for i in range(len(logits_lod[0]) - 1):
                ...
                # print(example['text'])
                # print(inference_tmp)
                # print(formated_result['spo_list'])
...
def predict(args, examples, exe, test_program, test_pyreader, graph_vars):
    ...
    while True:
        try:
            ...
            break
    time_end = time.time()
    return res, "[evaluation]elapsed time: %f s" % (time_end - time_begin)
...
def format_output(example, predict_result, spo_label_map,
    ...
    for cls_label in list(set(flatten_predict)):
        if 1 < cls_label <= 4 and (cls_label + 3) in flatten_predict:
    ...
    for id_ in subject_id_list:
        ...
        if id_ not in complex_relation_label:
            ...
            objects = find_entity(id_ + 3, predict_result)
